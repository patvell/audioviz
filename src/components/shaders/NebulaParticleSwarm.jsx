...
const NebulaMaterial = shaderMaterial(
  ...
  void main() {
    ...
    // Bass pulls everything to center
    pos -= dir * uLows * 3.0 * smoothstep(8.0, 0.0, dist);
    
    // Mids add turbulence
    pos += vec3(
      cnoise(noisePos),
      cnoise(noisePos + vec3(100.0)),
      cnoise(noisePos + vec3(200.0))
    ) * noiseAmp * uMids;
    
    vec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);
    gl_Position = projectionMatrix * mvPosition;
    
    // Point size reacts to highs and distance
    gl_PointSize = (4.0 + uHighs * 20.0 + cnoise(pos * 5.0) * 5.0) * (10.0 / -mvPosition.z);
  }
  `,
  // Fragment Shader
  `
  ...
  void main() {
    ...
    // Brighten based on highs and center proximity
    vec3 color = uColor + (vec3(1.0) * uHighs * 0.5);
    color *= smoothstep(10.0, 0.0, vDist); // darker on edges
...
export const NebulaParticleSwarm = ({ active }) => {
  ...
  const [positions] = useMemo(() => {
    ...
    for (let i = 0; i < 15000; i++) {
      ...
      pos[i * 3 + 2] = r * Math.cos(phi) * (Math.random() - 0.5) * 5; // flatten somewhat
    }
    return [pos];
  }, []);
  ...
  return (
    <points>
      ...
      <nebulaMaterial ref={materialRef} transparent={true} blending={THREE.AdditiveBlending} depthWrite={false} />
    </points>
  );
};
